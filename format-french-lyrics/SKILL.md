...
description: "Format French song lyrics for study from a paired French lyrics + Chinese translation file. Use when Codex needs to add standard French IPA, translations, and detailed Simplified Chinese grammar/vocabulary notes into a structured Markdown lyric sheet for French learning."
---
...
① 词或短语（IPA）：词性，中文解释；必要时说明词源、搭配、语域或歌词中的省略。
② 动词（原形 infinitif；当前形式 temps/mode/personne）：说明变位、含义和句中作用。
③ 语法结构：说明结构如何影响理解。
...
4. If the input is long or pairing is uncertain, use `scripts/build_lyrics_skeleton.py` to generate a skeleton and inspect warnings before filling IPA and notes.
5. Add standard French IPA under each French lyric line. Use contemporary Standard French pronunciation, including common liaison/enchainement only when it would normally be pronounced in careful singing or connected speech.
...
- Put the IPA immediately after the French line. Wrap it in slashes, for example `/ʒə tɛm/`.
- Use Standard French IPA, not English-style respelling or Chinese phonetic approximation.
- Mark elision and connected pronunciation naturally in IPA, for example `j'aime` as `/ʒɛm/`, `l'amour` as `/lamuʁ/`.
- Show nasal vowels with IPA symbols such as `/ɑ̃/`, `/ɛ̃/`, `/ɔ̃/`, `/œ̃/` when appropriate.
...
- Put the Chinese translation immediately after the IPA line.
...
- For very short interjections such as `Oh`, `Ah`, `Yeah`, output the line, IPA, translation, and separator without forced grammar notes unless context makes it useful.
...
Then replace each `<!-- phonetics -->` placeholder with standard French IPA and each `<!-- annotations -->` placeholder with detailed numbered notes.
...
- Every non-header lyric block has French, IPA, Chinese translation, then notes in that order.
- `---` separators are present between blocks.
- IPA uses standard French IPA symbols and slash notation.
